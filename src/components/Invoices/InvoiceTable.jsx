...
const InvoiceTable = () => {
  ...
  // Handle filter button click to filter invoices by status
  const handleStatusClick = (status) => {
    setFilterStatus(status);
  };

  // Filter invoices based on the status selected by the user
  useEffect(() => {
    const filtered = invoices.filter((invoice) => {
      if (filterStatus === "") return true; // Show all invoices when no filter is applied
      return invoice.status.toLowerCase() === filterStatus.toLowerCase();
    });
    setFilteredInvoices(filtered);
  }, [filterStatus]);

  // Handle the action buttons (view, edit, delete)
        ...
        <Box sx={{ display: "flex", marginTop: '30px', flexDirection: "row", position: "relative", width: "100%" }}>
          ...
          >
            <Table aria-label="invoice table">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider, fontWeight: "bold", backgroundColor, color: themes.palette.text }}>
                    Invoice ID
                  </TableCell>
                  <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider, fontWeight: "bold", backgroundColor, color: themes.palette.text }}>
                    Customer
              ...
              <TableBody>
                {filteredInvoices.map((invoice) => (
                  <TableRow key={invoice.id} sx={{ borderBottom: "1px solid", borderColor: theme.palette.divider }}>
                    ...
                    <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider }}>{invoice.customer}</TableCell>
                    <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider }}>{invoice.productName}</TableCell>
                    <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider }}>{invoice.brand}</TableCell>
                    <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider }}>{invoice.total}</TableCell>
                    <TableCell sx={{ border: "1px solid", borderColor: theme.palette.divider }}>{invoice.status}</TableCell>
